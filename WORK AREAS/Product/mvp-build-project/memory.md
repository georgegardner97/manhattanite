...
- **Favicon** — `app/icon.tsx` (next/og `ImageResponse`): bone serif roman **"M"** close to center on a rounded park tile, paired with a **round dot to its right, vertically centered on the M's level** (its midline) — an "M•" mark. The dot is **~14% of the tile height** (18% at 16px to survive). The M shifts a few percent left to make room but stays close to center. **The dot is DRAWN** (a border-radius circle), not the font's period glyph — a scaled-up serif period reads as noise at 16px, a drawn circle stays a clean deliberate dot. Roman, not italic (italic M smears at 16px). Deleted the scaffold's default `app/favicon.ico`. **Three real renders via `generateImageMetadata`** (64 / 32 / 16), not one downscaled. Verified at 64, 32, 16 in light + dark chrome. (Favicon iterated on 2026-07-21: period-less "M" → centered "M." pair → M + small satellite period → M + large baseline dot → **"M•", dot mid-level at ~14%** — this last is the locked design.)
