...
- **Favicon** — `app/icon.tsx` (next/og `ImageResponse`): bone serif roman **"M"** dead-center on a rounded park tile, with the **period demoted to a small satellite in the lower-right corner** (roughly the M's baseline, inset ~10% from the right edge). The M is the mark and is centered on its own; the dot is a signature, not part of the centering. Roman, not italic (italic M smears at 16px). Deleted the scaffold's default `app/favicon.ico`. **Three real renders via `generateImageMetadata`** (64 / 32 / 16), not one downscaled: at 16px the satellite period is drawn larger to survive the pixel grid. Verified at 64, 32, 16 in light + dark chrome. (Favicon iterated twice on 2026-07-21: period-less "M" → centered "M." pair → **M centered with the period as a lower-right satellite** — this last is the locked design.)
