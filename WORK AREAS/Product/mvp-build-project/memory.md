...
- **Favicon** — `app/icon.tsx` (next/og `ImageResponse`): bone serif roman **"M."** — the period is kept at every size — on a rounded park tile. Roman, not italic (italic M smears at 16px). Deleted the scaffold's default `app/favicon.ico`. **Two real renders via `generateImageMetadata`** (32px + 16px), not one downscaled: at 16px the period is enlarged (its own span at a larger font) so the dot stays a legible point, and the M shrinks a touch to fit the pair without clipping. The "M." pair is nudged left ~a period's width so the M sits on the tile's optical center and the dot hangs into the right margin. Verified crisp at 32 and 16, light + dark chrome. (George revised 2026-07-21: was a period-less "M"; now "M." with the dot at all sizes.)
