...
- **Favicon** — `app/icon.tsx` (next/og `ImageResponse`): bone serif roman **"M."** on a rounded park tile — a period resting on the M's baseline, like a normal period after the letter, just slightly larger than a natural one. The dot is **~9% of the tile height** (13% at 16px to survive). The M stays close to center, shifted a few percent left to make room. **The dot is DRAWN** (a border-radius circle), not the font's period glyph — a scaled serif period reads as noise at 16px; a drawn circle stays a clean dot at every size. Roman, not italic (italic M smears at 16px). Deleted the scaffold's default `app/favicon.ico`. **Three real renders via `generateImageMetadata`** (64 / 32 / 16), not one downscaled. Verified at 64, 32, 16 in light + dark chrome. (Favicon iterated on 2026-07-21: period-less "M" → centered "M." pair → M + small satellite period → M + large baseline dot → M• mid-level dot → **"M." — normal period on the baseline, ~9%** — this last is the locked design.)
