...
- **Favicon** — `app/icon.tsx` (next/og `ImageResponse`): bone serif roman **"M"** close to center on a rounded park tile, paired with a **large confident round dot** (~20% of the tile height, the "J." favicon register) sitting on the M's baseline in the lower right. The M shifts a few percent left to make room but stays close to center. **The dot is DRAWN** (a border-radius circle), not the font's period glyph — a scaled-up serif period reads as noise at 16px, a drawn circle stays a clean deliberate dot. Roman, not italic (italic M smears at 16px). Deleted the scaffold's default `app/favicon.ico`. **Three real renders via `generateImageMetadata`** (64 / 32 / 16), not one downscaled; at 16px the dot is nudged toward 22% so it still reads as a circle. Verified at 64, 32, 16 in light + dark chrome. (Favicon iterated on 2026-07-21: period-less "M" → centered "M." pair → M centered + small satellite period → **M with a large confident round dot** — this last is the locked design.)
