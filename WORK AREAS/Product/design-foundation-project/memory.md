...
## 2026-07-21 — Mobile pass (emulated half) SHIPPED

- Claude Code ran the 390×844 DPR-3 audit (375×667 spot-check included) across every reachable route and fixed the trap list within the existing system. The fixes, all CSS/markup only: **.mh-input → 16px under 860px** (kills iOS focus-zoom; landing email row bumped the same way), **hero 92vh → 92svh** with vh fallback (new `.mh-vh-hero` utility; ContactModal's 90vh cap → `.mh-vh-modal` likewise), **viewport-fit=cover + safe-area insets** (gutter, hero chrome, footer bottom — the hero nav lost its negative-margin trick and is now a full-bleed `mh-gutter` child, pixel-identical on desktop), **the three hand-rolled hover rules gated behind `(hover: hover)`** (Tailwind v4 already gates its own hover: variants — verified in the compiled CSS), **`.mh-tap` utility** (coarse-pointer-only ::before hit-area to 44×44px, zero layout effect) applied to nav/hero/footer/auth links, ArrowLink, card-footer actions, account menu, modal close — and baked into `.mh-link` itself, and **the browse filter row now scrolls the active chip into view on load** (FilterRow extracted to a small client component, `ListingsFilterRow`; on `?type=other` the active chip used to sit entirely offscreen).
- **Verified:** every logged-out route swept at 390 and 375 with zero horizontal overflow (checked programmatically in real mobile emulation — mobile UA, touch, DPR 3), desktop spot-checked (hero, nav hover fill, browse rail all unchanged), `npm run build` clean. The "after" screenshot set lives in `outputs/mobile-pass-screens-2026-07/`.
- **Logged-in screens** (post form, profile ×2, mine, admin) could not be reached locally — Stage 0 is still blocked at Cloudflare (localhost not on the widget's allowed hostnames) — so they were audited at code level; they share `.mh-input`, `PageShell` and the fixed utilities, and all have `max-[860px]` fallbacks. George's device pass covers them live.
- **Flagged, not fixed:** hero photo (1400px source) is soft at DPR 3 — already a `TODO(phase-4)` in `app/page.tsx`; listing photos are plain `<img src>` with no `srcset`/`sizes`, so phones download the full-size original — an image-transform slice for later, not a CSS fix.
