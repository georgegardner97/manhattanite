...
## 2026-08-27 — Both bugs fixed, walkthrough batch MERGED and deployed (`2e80c65`)

- George's call: fix them and merge. Both fixes are one-liners in spirit; both were verified by driving the real form against the production database rather than by inspection.
- **Fix 1** — `ClImageUpload` now writes `images` as the array of strings the server actions have always checked for. Confirmed live by reading the deployed chunk off manhattanite.com: `name:"images",value:JSON.stringify(r.map(e=>e.path))`. Listings with photos can be posted and edited again.
- **Fix 2** — `create.ts` and `update.ts` read `neighborhood` for furniture too, so a furniture listing keeps it through an edit. `details` is still rebuilt wholesale (deliberate: a category switch must leave no stale keys), so the seed-only `tags`/`category` are still dropped — nothing reads them, checked before leaving it.
- **The proof, in one action:** a new furniture listing posted with a photo, a blank price and a Tribeca neighborhood landed as `price_cents` NULL, `details.neighborhood` "Tribeca", `images` in the stored `{ path }` shape, `status` pending — and `/admin/moderation` showed both "No price" and the neighborhood row. Test row and both orphaned Storage objects removed by exact path; database diffed byte-identical to the session's start.
- **Merged `--no-ff` as `2e80c65`** so the batch reverts as a unit (`git revert -m 1 2e80c65`). `audit:rls` 59/59; `audit:gates` 0 failures locally and against production.
- **Live design checks, all five of George's notes confirmed on manhattanite.com:** header inner box and `<main>` both 100 → 1500 at 1600px (the 80px inset is gone); search box on Browse; furniture cards read "FURNITURE" while apartments keep their neighborhood kicker; the NEIGHBORHOOD filter group appears under Apartments only, and a stale `?hood=` leaves the URL on category switch; `/search?q=lamp` → 308 → `/listings?q=lamp`. Guest name rule still holding — "Vouched by a member", no real names.
- **Design lesson, carried forward:** both bugs were a control that renders but whose action never reads it. That is a design-integrity failure as much as an engineering one — the form promised two things it did not keep. Worth a sweep of every control against its action before Slice 3b, which is now the only Classifieds work left.

## 2026-08-27 (later) — Profile menu rebuilt, sort control removed, and the takedown button that never worked

**Five things this session: four George briefed, plus one he found while trying to use the site. Built and audited, not merged.**

- **The session's opening gate failed, and that turned out to be the story.** The brief asked me to confirm a test listing was gone before starting. It was still live on manhattanite.com. I stopped and said so. He came back with the reason: the takedown button on the edit screen does nothing. He had been trying to remove it himself and the product would not let him.

- **`ClRemoveListing` was nested inside the post form's `<form>`.** A `<form>` inside a `<form>` is invalid; the browser drops the inner one and the submit button re-associates with the outer form, so "Yes, take it down" ran `updateListing` — it saved the edit and archived nothing. Never worked, shipped with the Classifieds merge, live a fortnight. Fixed by returning a fragment with the component a true sibling after `</form>`.

- **Both files already carried a comment stating that exact rule.** Both were right; the code did the opposite. **A comment is documentation, not enforcement** — and this is the second nested-interactive-element trap in this component family, after the button-inside-a-link on the card (which, checked this session, is genuinely correct). The guard is now an assertion in `audit:gates` that fetches the real edit page as a real member and counts unclosed `<form>` tags before the control. Its teeth were proved by reverting to the buggy markup and watching it fail.

- **`/listings/mine` had no entry point anywhere in the Classifieds system — the third instance of one pattern.** Its only doors were `SiteFooter` and `AccountMenu`, both editorial; after the migration the only `(ed)` routes left are the four admin pages, so the only way into a member's own listings rendered where only the founder goes. `/admin`, then `/search`, now this. **When a design system is retired, the surviving routes need their entry points re-homed. "The route still works" is not the same claim as "someone can get there."** For a design project this is the sharpest version of the lesson: a screen with no door is not a screen, however finished it looks. I swept the rest by crawling from a signed-in member's real starting points rather than reading the route list — the only orphan left is `/invite`, which is a deliberate one waiting on the growth-loop decision.

- **The profile rail is now Account · My listings · Saved · Vouching.** "Leaving" stopped being a peer of the other three — a strange fourth thing to offer someone — but the two things inside it survived: the product's only sign-out is a quiet POST control at the foot of the Account panel, and the account-closure line sits beneath it as muted text keeping `id="leaving"`, because `/terms` and `/privacy` both promise closure and deleting it would leave the policy overclaiming.

- **Vouched names now click through, in both lists.** They were plain text here while the same person's name is already a link in a listing byline — one of the two was wrong. Both directions now render through a single shared row component, so they cannot drift apart the way they just had. Whole row is the target, avatar placeholder stays `aria-hidden`, and the "brought you in" tag is passed only for the sponsor direction so it can never read backwards on someone you brought in yourself.
