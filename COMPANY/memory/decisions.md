...
- **An admin may edit any listing, and the scope is CORRECTION, not rewriting.** (2026-08-28, George, in the Slice 3b brief) The purpose is spelling, a factual error, a photo that should not be the cover. Never a rewrite of what a member said under their own name. Three things hold the line rather than a policy sentence: the admin write set is IDENTICAL to the member one (an admin cannot reach a field a member could not have set); the byline columns are outside it entirely; and every correction stamps `corrected_by` / `corrected_at` and shows the owner **"Corrected by Manhattanite"** on their own listing. **Corrections are recorded, never silent** — on a network whose product is that a name means something, an invisible rewrite of what somebody wrote under their own name is the wrong default even when the intent is a spelling fix. No email per typo: the point is that a member can see a change happened, not that they get pinged. No person is named to the member either, which is what later made it safe for the corrector pointer to be nullable.
  - **A correction must not re-pend the listing.** Pulling a live listing off the site so George could approve his own typo fix would be a worse bug than the typo. Verified: an admin correction leaves a published listing published.
  - **No photo cropper.** There is none anywhere in the product; what admin gets is what an owner gets — remove, add, reorder so a different photo is the cover. A cropper is its own piece of work for everyone, not an admin-only feature.
  - **Admin take-down requires a recorded reason**, in the form, the server action and the database function. A take-down with no recorded reason is what turns a trust layer into an opinion.
  - **The RLS policy was NOT loosened to achieve any of this.** Listings stay owner-only for writes; admin access exists at exactly two named SECURITY DEFINER functions (0028). **The policy is the wall, the function is the door** — a phrase worth keeping, because the tempting shortcut was to widen the policy and it would have opened the wall to every future code path at once.
...
- **George's live-site walkthrough, settled and shipped the same day.** (2026-08-27) Five notes taken on manhattanite.com the day after the merge; four decisions in them, all implemented on `classifieds-walkthrough-fixes` as one batch so the lot reverts together.
  - **A listing may have no price at all.** Blank stores NULL and renders as no price line anywhere a member or a visitor looks. **NULL is "no price"; 0 is NOT** — free is a real asking price on a classifieds board and has to stay sayable, so nothing may branch on falsiness. Two deliberate exceptions say "No price" out loud, `/admin/moderation` and the post form's Review step, where silence would read as a broken row rather than a choice. A price filter excludes unpriced listings; the Price sort puts them last. This is option B from the post-form brainstorm **minus the price modes** — a plain blank, chosen as the first move rather than a picker.
  - **Card kickers lead with the CATEGORY for everything that is not an apartment.** A $220 coffee table headlined "LOWER EAST SIDE" is rental-portal grammar, and was the second-largest reason browse read as a rental site. Apartments still lead with their neighborhood. **The largest reason is untouched and is not code: 12 of 20 seed listings are apartments.**
  - **Search moved onto Browse; `/search` retired to a permanent redirect.** It was fully built, was the same gated read with a text term added, and nothing in the product linked to it — the only way in was to type the URL. This also settles the nav question left open when Saved left: the vacated slot stays vacant.
  - **Saved is not a main-nav item.** Route, save pills and the screen are unchanged; it is found from `/profile` now. Nav is Browse · Profile; the phone's tab bar is Browse · Post · Profile.
  - **Wanted / "looking for" listings are PARKED**, along with price modes and new categories. Revisit once real listings show whether they are needed.
  - **The neighborhood filter is an apartments control** (not strictly a strategic call, but it is the one George raised first). Held by a single predicate so switching category drops a stale filter out of the URL instead of applying it invisibly.

- **A design decision must never double as a data value.** (2026-08-27, found while implementing the above) `placeOf()` was simultaneously the string printed on a card AND the value the neighborhood filter and the search haystack compared against, so changing what a card SAYS would have silently changed what search FINDS — a Tribeca coffee table would have stopped answering "tribeca", with nothing failing anywhere. Split into `neighborhoodOf()` (data) and `placeOf()` (display) before either was touched. **The general rule: when one function is both what a person reads and what the machine matches on, separate them before you change either.**

- **A member says why a listing is coming down, and the answer IS the confirmation.** (2026-09-02, George: *"it would be useful for when a listing is taken down for there to be a choice — was this listing fulfilled? So that we have data on what was working and what wasn't."*) Four values, not a boolean — `found_here`, `found_elsewhere`, `withdrawn`, `no_luck` — because a fulfilled yes/no collapses the three ways of not succeeding into one and destroys both facts worth having. **`found_here` and `found_elsewhere` must never be merged into "fulfilled":** a member who sells the table to their sister fulfilled the listing without the network doing anything, and counting it as a match would flatter us into believing something untrue.
  - **The mechanism is the point, not the question.** The four buttons replace the single "Yes, take it down", so picking one is the only way to complete the takedown. A skippable question after the fact gets answered by nobody; a required extra step gets answered dishonestly by people trying to leave. This costs the member zero extra taps and should approach a 100% response rate.
  - **NULL is meaningful and will stay common** — archived before 2026-09-02, taken down by an admin, or withdrawn while still pending. **The denominator for any rate is member-archived, previously-published listings, not every archived row.** Nothing renders a null as "Unknown".
  - **A pending listing is not asked**, because a listing nobody could see cannot have found anyone. **The admin take-down deliberately records no outcome** — a founder removing somebody's listing is a moderation event, not the member reporting a result.
  - **No dashboard, chart or rate was built.** With one published listing a percentage is noise. Aggregation is a separate decision for when there is something to aggregate.
...
- **THE WORD IS "VOUCH", NOT "SPONSOR", EVERYWHERE A MEMBER CAN READ IT.** (2026-08-31, George) He raised it unprompted: *sponsor* carries financial and recovery meanings that pull against a trust product — a sponsor pays for something, or holds you accountable in a program; neither is what a member does here. Radio Hong Kong-Pacific's *seconder* was the prompt; the club register (*proposer / seconder*) and *introduced by* were both considered and rejected — the first makes two names non-interchangeable when the model allows any number, the second is softer than the trust claim being made. **Vouch won partly because the product already used it in half its screens** (`/members/[id]` said "Vouched by", `/profile` had a Vouching panel, the members-only wall said "a member has to vouch for you first") while the byline said "sponsored by" — so this ended an inconsistency rather than starting a rename. The byline is now **"Listed by Claire · vouched for by Dan"** — he corrected "vouched by" to "vouched for by" on reading it back, and the longer form is right: you vouch FOR a person, and the short form reads as though Dan is the one being vouched.
  - **The rename is COPY-ONLY and that is deliberate.** The `sponsorships` table, `sponsor_names` / `sponsor_id` columns, the RPCs and every identifier in the code keep their names. Renaming a live schema to change a noun buys nothing a member can see and risks the byline every listing depends on. Anyone reading the code should expect the two vocabularies side by side; `lib/listings/byline.ts` carries the note saying why.
  - **No noun for the person.** *Voucher* reads like a coupon, so the copy says "the member who vouched for you" wherever it needs to name the role. Slightly longer, and the only alternative was worse.
  - Terms and Privacy changed with it ("Membership and vouching"; vouching is a social act of trust, not a legal guarantee), and `COMPANY/voice-and-copy.md` was updated in the same pass so the copy library cannot drift back.
...
- **Monetisation analysis delivered, pricing not yet confirmed.** (2026-08-27) New project: `WORK AREAS/Product/monetization-project/` (logged here as a system change). Recommendation on the table: switch-on trigger becomes an observation (repeat posting + reply rates) rather than a member count; apartments $99/30 days free-until-rented; furniture free forever; jobs $75 at v1.5; payment authorised on submit and captured on approval. Supersedes the playbook's "1 free listing/month then $25". **The 2026-07-14 broker-fee rationale is void** — NYC's FARE Act (in effect 2026-06-11) moved broker fees onto landlords; the anchor is now what a lister pays (StreetEasy FRBO $249/2wks, Listings Project $47/wk). Awaiting George's call on price and on whether furniture is free permanently. Detail: `WORK AREAS/Product/monetization-project/outputs/Manhattanite_Pricing-Model_v1.md`.
...
## Positioning — the pitch (2026-09-02)

- **THE PITCH IS TWO DESCRIPTIONS, NOT ONE.** (2026-09-02, George) Spoken wins on punch, written wins on dignity, and one line cannot do both. Spoken default: *"It's an invite-only listings site for New Yorkers. A member has to vouch for you to get in — and if you behave badly, they go too."* Written: the four-paragraph invitation in `Manhattanite_One-Line-Pitch_v2.md`. This closes the decision opened 2026-07-14 after the Marwan meeting and deferred again on 1 August.
- **THE MECHANISM IS SHARED LIABILITY, NOT A NAME ON A PROFILE.** (2026-09-02, George) "If you break the rules, they go too" is a consequence; "their name stays on your profile" was only a label. This is now the centre of every pitch surface, and the PRIDE motive was rewritten from *whose name means something* to *whose judgement you'd trust*.
- **CRAIGSLIST IS THE COMPARISON, NEVER THE SELF-DESCRIPTION.** (2026-09-02) Established from what the comparators actually do: Radio H-P's founder says "an honourable and trusted network" while the press says "a posh Craigslist"; Gens de Confiance says "a trustworthy ad site" while everyone else says classified ads. Let the listener reach for the comparison and agree warmly; never volunteer it. Retires *"It's Craigslist, if everyone on it was vouched for."*
- **"CLASSIFIEDS" IS NOT A SELF-DESCRIPTION EITHER, AND THE FIX IS VERBS, NOT A SYNONYM.** (2026-09-02, George raised it) Craigslist's own strapline is "local classifieds and forums", so the word belongs to them. GDC's answer is "buy, sell, or rent with confidence". Consistent with `voice-and-copy.md`, which already fixes the product noun as *Listing — never post or ad*.
