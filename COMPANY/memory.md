...
**`Test - Ignore` IS STILL LIVE on manhattanite.com** — published, authored by George, visible to a logged-out visitor, published count 21 not 20. The session's opening gate was to confirm it was gone; it was not, so work stopped and he was told. **The reason it is still there is the bug he then reported:** the takedown button on the edit screen never worked. That is now fixed, so once this is deployed the edit screen will actually remove it.

**THE TAKEDOWN BUTTON NEVER WORKED, AND A COMMENT IS NOT ENFORCEMENT.** `ClRemoveListing` renders its own `<form>` and was nested inside the post form's `<form>`. Invalid HTML — the browser drops the inner form and the submit re-associates with the outer one, so "Yes, take it down" ran `updateListing` and saved the edit. Live for a fortnight, shipped with `4759502`. **Both files already carried a comment saying the component must be a sibling; both were right and the code did the opposite.** Fixed by making `ClPostForm` return a fragment with the component after `</form>`. The guard is now `checkNotInForm()` in `audit:gates` — it fetches the real edit page as a real member and counts unclosed `<form>` tags before the control. **Its teeth were demonstrated, not assumed:** reverted to the buggy markup, both assertions failed; restored, both passed. `test-edit-archive.ts` is deliberately not the home for it — that harness drives `archiveListing` at the database and passed the whole time the button was dead.

**THE ORPHANED-ROUTE PATTERN IS THE LESSON WORTH KEEPING, AND THIS WAS THE THIRD INSTANCE.** `/listings/mine` worked and nothing linked to it: its only doors were `SiteFooter` and `AccountMenu`, both editorial, and the only surviving `(ed)` routes are the four admin pages — so the only way into a member's own listings rendered where only the founder goes. `/admin` before the merge, `/search` this morning, `/listings/mine` now. **When a design system is retired, the surviving routes need their entry points re-homed. "The route still works" is not the same claim as "someone can get there"** — and no build, type check, `audit:rls` or `audit:gates` can catch it, because all of them address routes by URL, the one thing a stranded page still has. The check that catches it is a crawl from a signed-in member's actual starting points, following only real links.

**Sweep result: one genuine orphan left, `/invite`, and it is deliberate.** The page documents its own state and is waiting on the growth-loop decision, not on a link — left alone, because adding an invite entry point is a product call. Everything else unreached is unreached on purpose (`/thank-you`, `/profile/edit`, `/search`'s 308, the three email-arrival routes, `/apply` and `/signup` for guests). **Low-severity flag:** `/listings/[id]/contact` has no `href` anywhere — contact is the "Get in touch" modal — so its own comment claiming "It is linked directly" is stale, and with JavaScript off there is no path to contact a lister.

**`/profile` is now Account · My listings · Saved · Vouching.** "Leaving" stopped being a menu item, but neither thing inside it was lost: the product's **only** sign-out is now a quiet POST control at the foot of the Account panel, and the account-closure email route sits beneath it as muted text keeping `id="leaving"`. That paragraph is load-bearing — `/terms` and `/privacy` both promise account closure, and deleting it would leave the policy overclaiming. Vouched names in **both** directions now link to `/members/[id]`, through one shared `ConnectionList` so the two lists cannot drift apart again; whole row is the target, avatar stays `aria-hidden`, and "brought you in" is passed only for the sponsor direction so it can never read backwards.

**THERE IS NO SORT CONTROL ON BROWSE, AND THAT IS A DECISION** (George, 2026-08-27). Sorting by price ranks the network cheapest-first, which is the Craigslist frame; the min/max boxes already serve the budget question. With price gone "Newest" was a control with one option, so the row went and the result count stays. `?sort=price` is IGNORED, not an error. **This deleted the "unpriced listings sort LAST" rule added earlier the same day — correctly, because it existed only to serve the price sort. Do not re-add it thinking it was lost by accident.** Blank prices still render as no price line and are still excluded by a min/max filter; neither changed.

**Found in passing, not fixed, George's call:** the edit screen says "It goes back through review before it's live again", but `updateListing` never writes `status`, so an edit to a published listing goes straight back live. The copy promises a moderation step that does not happen.

**Results:** build clean, `tsc` clean, lint unchanged at the 4-error baseline, `audit:rls` 59/59, `audit:gates` 0 failures locally. Against production it reports **2 failures — the two nested-form assertions — which is the correct answer**, because prod is still the buggy build. They go green on deploy.
