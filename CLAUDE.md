...
- **THE MEMBER-FACING WORD IS "VOUCH", NEVER "SPONSOR"** (George, 2026-08-31). Members are *vouched for*; the byline reads "Listed by Claire · vouched for by Dan" — **"vouched FOR by", never "vouched by"** (George, 2026-08-31, on reading it back); the profile panel is Vouching. *Sponsor*, *sponsored* and *sponsorship* carry financial and recovery meanings that pull against a trust product and must not appear in any string a member, applicant or visitor can read — Terms and Privacy included. **The CODE still says sponsor and that is deliberate**: `sponsorships`, `sponsor_names`, `sponsor_id`, the RPCs and every identifier keep their names, because renaming a live schema to change a noun risks the byline every listing depends on and buys nothing visible. Expect both vocabularies in the same file. There is no noun for the person — *voucher* reads like a coupon — so copy says "the member who vouched for you".
- **NO DASHES IN ANY STRING A HUMAN READS** (George, 2026-09-15, standing rule). No em dash, no en dash, no `&mdash;`, no `&ndash;`, and no hyphen inside an ordinary English word. George does not like them and does not want to see one anywhere on the site. This covers every page, form label, placeholder, button, error message, admin screen, page title, share-card text and transactional email. Rewrite the sentence rather than swapping the character: a full stop, a comma, a colon, or brackets. Hyphenated words get unhyphenated or reworded ("invitation only", "fair housing", "members only", "record keeping", "apply again"). For a title separator use the middot `·`, which the product already uses ("Sign in · Manhattanite"). **What is exempt:** code identifiers, CSS class names, Tailwind utilities, HTML attributes, font file names, web addresses, migration file names, and code comments. Those are not copy and renaming them breaks things. A dash inside `className`, `href`, an `import`, or a `//` comment is fine; a dash inside a sentence is not. The full sweep was done 2026-09-15 across 39 files; if you are adding copy, do not reintroduce one.
- **THE WORDMARK IS A DRAWING AND THE SITE IS ONE FONT** (George, 2026-09-16; supersedes Concept D). The mark is a handwritten script "Manhattanite." outlined from Cedarville Cursive (OFL 1.1), rendered as inline SVG by `app/components/Wordmark.tsx` with `fill="currentColor"`. Say "outlined from Cedarville Cursive (OFL 1.1)", never "original lettering". **Size it by HEIGHT (`h-[34px]`), never by font size**, which does nothing to an SVG. The full stop is part of the mark and is never dropped (the favicon tile is the one exception). **Everything else is Instrument Sans, the only font `app/fonts.ts` loads**: do not bring back a serif, Newsreader or Inter. **Dark green #13241B belongs to the favicon tile, the apple touch icon and the share card only**, never the site UI. The share card (`app/opengraph-image.tsx`) repeats the landing's line "A private marketplace for New York.", so change both together. Rules in `COMPANY/brand-guide.md`.
...
10. **THE EDIT SCREEN NOW SAYS WHAT SAVING ACTUALLY DOES, PER STATUS** (George, 2026-08-31 — the copy was wrong, not the behaviour). It used to promise every editor "It goes back through review before it's live again", while `updateListing` deliberately never writes `status` (the 0017 trigger waves content edits through), so an edit to a published listing has always gone straight back live. The single line is now `EDIT_NOTES` in `ClPostForm.tsx`, one string per status: draft says make the changes and send it back, pending says it is still waiting on review, published says the edits go live as soon as you save, archived says saving will not put it back on the site. **If an edit is ever made to re-enter review, `EDIT_NOTES` has to change in the same commit** — it is the only place the product tells a member what saving will do. Admin mode still suppresses the whole block (`ClAdminShell` has already set the scope).
...
15. **THE EXAMPLE TAG IS OFF THE CARD AND THE LISTING PAGE, AND IT HAS A DEADLINE** (George, 2026-08-31). The chip had been kept as a trust requirement — nobody should mistake a seed listing for a live deal — and **that reasoning has not changed; what changed is who is looking.** Nobody but George can reach the site, he knows which listings are his own seed content, and the chip cost the byline a second line on every card.
    - **BEFORE THE FIRST INVITATION (wave one is 7-13 Sep in timeline v3) EITHER THE TAG COMES BACK OR THE SEED LISTINGS GO.** George's plan is the latter. A stranger must never meet an unlabelled example. This is the whole condition attached to the removal, and it is written into the header of `ClListingCard.tsx` as well as here.
    - **`is_example` is untouched** — still on the row, still selected, still on `ClCard`. Only the chip stopped rendering, so restoring it is one JSX block, and the seed rows stay findable by the flag when it is time to delete them.
    - **Removed from the card and the detail page TOGETHER**, on purpose: a tag on the card that vanishes on the detail page (or the reverse) is worse than no tag. **`ClLandingCard` KEEPS its chip** — it renders on nothing today (note 13) and is held for that revert, so the tag stays with the code it would come back with.

16. **THE CARD'S META LINE IS A BYLINE AND A DATE, NOT ONE STRING** (2026-08-31, the space the tag freed). `cardMeta()` used to return "… · 4 days ago" as one blob, so the card could only render it as one run of text — and **a member's byline is wider than a card** (measured: a card is ~300px at a 1280px viewport, a guest's "Vouched for by a member" costs ~230px and fits, "Listed by Lila · vouched for by George Gardner" costs ~345px and does not). It wrapped, stranding "ago" on its own line, and where it broke depended on the lister's name, so no two cards in a row agreed.
    - **`cardMeta()` now returns the byline ALONE**; the date travels beside it as `ClCard.when`. **Nothing about who is named changed** — that decision is still made in exactly this one function, so CLAUDE.md note 0a stands and `npm run audit:gates` still holds it.
    - **The byline truncates; the date never does.** `min-w-0 truncate` on the byline, `shrink-0` on the date. What is lost on a long name is the tail of the name, never "4 days ago" — the part that says whether a listing is stale.
    - **An empty `meta` means date-only**, which is what `/members/[id]` passes (every card there has the same author). The separator is rendered by the card rather than baked into either string, so a date-only card does not open with a stray "·".

17. **A MEMBER HAS A PROFILE BECAUSE THEY ARE A MEMBER, NOT BECAUSE THEY POSTED** (George, 2026-08-31; migration `0026`, REWRITTEN AND APPLIED TO PROD 2026-08-31). `/members/[id]` was built entirely from the denormalized listing byline (`author_name`, `sponsor_names`, migration 0006) — a genuinely elegant choice, because it showed no fact that was not already printed on every card. **Its failure mode was invisible until someone clicked: a member with no PUBLISHED listing had no name the page could read, so it answered `notFound()`.** That is most new members, and it is exactly the person you are most likely to look up — George hit it clicking from his own vouching list on `/profile` to Emma Kanne. A 404 on a real member is the worst possible answer in a product whose entire claim is that you can see who someone is and who stands behind them.
    - **`0026` is the decision, and it is a privacy decision made deliberately** (George, 2026-08-31): to a SIGNED-IN viewer, a member name now buys `name, avatar_path, neighborhood, bio, linkedin_url, member_since, sponsor_names` and nothing else. Not email (contact stays on the logged, forwarded 0011 path), not role, not `sponsor_id`, and nothing at all about a non-member — `get_member_profile()` returns zero rows for a Tier 1 account, so `notFound()` now means one clean thing: **this id is not an approved member**.
    - **SPONSORS COME FROM `sponsorships`, NOT FROM LISTINGS**, for the same reason the name does. The 0026 rewrite adds `sponsor_names` to the return for this. Still NOT exposed: how many people a member has vouched FOR, which is the most socially loaded number on the screen and is its own decision.
    - **`0026` was rewritten in place rather than superseded by a new number** — it had been written in Slice 1 and never applied, so nothing depended on the old signature.
    - **The page degrades rather than breaks while 0026 is unapplied.** Migrations here are hand-run in the SQL editor, so the code can ship first: if the function is missing the page falls back to the old listings-derived name and the old 404, and starts working the moment the SQL runs. Do not remove that fallback casually — it is what makes "code first, SQL later" safe on this route.
    - `linkedin_url` is free text a member typed and this page shows it to other members, so it renders as a link ONLY when it resolves to a `linkedin.com` host. A profile field is not a place to hand someone an arbitrary outbound link on our say-so.
    - Neighborhood comes from the member row, not from their listings — a listing neighborhood is where they are selling something, not where they live.
    - Generalizes note 8 ("a route that works is not a route someone can reach"): **a route someone can reach is not a route that answers**. `audit:gates` checks routes by URL and would have passed this the whole time, because George is the one account on the site with listings.
