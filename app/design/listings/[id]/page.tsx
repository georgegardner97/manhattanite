...
import { TEASER_LIMIT } from "@/app/design/listings-read";
import AppHeader from "@/app/design/AppHeader";
import SaveButton from "@/app/design/SaveButton";
import ClContactModal from "@/app/design/ClContactModal";
import ClGallery from "@/app/design/ClGallery";
...
const TYPE_HREF: Record<ListingDetail["type"], string> = {
  apartment: "/design/browse?type=apartment",
  furniture: "/design/browse?type=furniture",
  other: "/design/browse?type=other",
  service: "/design/browse?type=service",
};
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-[1100px] px-[clamp(16px,2.4vw,28px)] pt-[22px] pb-[clamp(32px,4vw,56px)]">
        ...
        >
          <Link href="/design/browse">Browse</Link>
          ...
          {neighborhood && (
            <>
              <span aria-hidden="true">/</span>
              <Link
                href={`/design/browse?hood=${encodeURIComponent(neighborhood)}`}
              >
                {neighborhood}
              </Link>
            </>
